      ...
      <div className="p-6 lg:p-8">
        <div className="animate-pulse space-y-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[...Array(8)].map((_, i) => <div key={i} className="h-28 bg-slate-200/70 rounded-lg border border-slate-200"></div>)}
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Growth & Investment</h1>
          <p className="text-slate-500 mt-1 text-sm">Business growth analysis and investment recommendations</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex bg-slate-100 rounded-lg p-0.5 border border-slate-200">
            <button onClick={() => setMode('real')} className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${mode === 'real' ? 'bg-primary-600 text-white' : 'text-slate-600 hover:text-slate-900'}`}>
              Real Data
            </button>
            <button onClick={() => setMode('synthetic')} className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${mode === 'synthetic' ? 'bg-primary-600 text-white' : 'text-slate-600 hover:text-slate-900'}`}>
              Demo Data
            </button>
          </div>
        </div>
      ...
      {/* Data Source Label */}
      <div className={`mb-6 p-3 rounded-lg border text-sm ${data.source === 'synthetic' ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-emerald-50 border-emerald-200 text-emerald-700'}`}>
        <span className="font-medium">{data.label}</span>
        <span className="text-slate-500 ml-2">— {data.disclaimer}</span>
      </div>
          ...
          <div className="flex items-center gap-6">
            <div className="relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-2xl font-bold text-slate-900">{data.growth_score.score}</span>
              </div>
            </div>
            <div className="flex-1">
              <h3 className="text-lg font-bold text-slate-900 mb-1">Growth Score: {data.growth_score.score}/100</h3>
              <p className="text-sm text-slate-600 mb-3">{data.growth_score.summary}</p>
              <div className="flex flex-wrap gap-2">
                {data.growth_score.factors.map((f, i) => (
                  <span key={i} className={`text-xs px-2 py-1 rounded ${
                    f.impact === 'positive' ? 'bg-emerald-50 text-emerald-700' :
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div className="card p-5">
          <h3 className="text-sm font-semibold text-slate-900 mb-4">✅ What is Going Well</h3>
        ...
        <div className="card p-5">
          <h3 className="text-sm font-semibold text-slate-900 mb-4">🎯 Recommended Next Actions</h3>
          <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {data.growth_opportunities.map((opp, i) => (
            <div key={i} className="bg-slate-50 rounded-lg p-4 border border-slate-200">
              ...
              <p className="text-xs text-slate-600 mb-2">{opp.message}</p>
              {opp.metric && <span className="text-xs font-bold text-primary-700">{opp.metric}</span>}
            </div>
          ))}
        </div>
      </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`text-[10px] px-1.5 py-0.5 rounded ${rec.confidence === 'High' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                        {rec.confidence} Confidence
                      </span>
                    </div>
                    <p className="text-xs text-slate-600 mb-1"><span className="text-slate-500">Reason:</span> {rec.reason}</p>
                    <p className="text-xs text-slate-600"><span className="text-slate-500">Expected impact:</span> {rec.expected_impact}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {data.investment_allocation.allocations.length > 0 && (
        <div className="card p-5 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-slate-900">Suggested Investment Allocation</h3>
            <span className="text-xs font-bold text-primary-700">Budget: ₹{formatNumber(data.investment_allocation.total_budget)}</span>
          </div>
          <p className="text-[10px] text-amber-600 mb-4">⚠️ {data.investment_allocation.disclaimer}</p>
          <div className="space-y-3">
            {data.investment_allocation.allocations.map((alloc, i) => (
              <div key={i}>
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-slate-600">{alloc.category}</span>
                  <span className="text-slate-900 font-medium">₹{formatNumber(alloc.amount)} ({alloc.percentage}%)</span>
                </div>
                <div className="w-full bg-slate-100 rounded-full h-2">
                  <div className="bg-primary-500 h-2 rounded-full transition-all" style={{ width: `${alloc.percentage}%` }} />
                </div>
                <p className="text-[10px] text-slate-400 mt-0.5">{alloc.reason}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {data.category_performance.length > 0 && (
        <div className="card p-5 mb-6">
          <h3 className="text-sm font-semibold text-slate-900 mb-4">Category Performance</h3>
          <div className="space-y-3">
                ...
                <div key={i}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-xs text-slate-500">{cat.units_sold} units</span>
                      <span className="text-xs text-emerald-600">{cat.avg_margin}% margin</span>
            ...
            {data.monthly_data.map((m, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col justify-end items-center gap-1 group relative">
                  <div className="absolute bottom-full mb-2 hidden group-hover:block bg-slate-900 border border-slate-700 rounded px-2 py-1 text-[10px] text-white whitespace-nowrap z-10">
    ...
    <div className={`bg-gradient-to-br ${colorMap[color] || colorMap.blue} border rounded-lg p-4`}>
      <div className="flex items-start justify-between">
        <div>
          ...
          {trend && <p className={`text-[11px] mt-1 ${trendUp ? 'text-emerald-600' : 'text-slate-500'}`}>{trendUp && '↑ '}{trend}</p>}
        </div>
