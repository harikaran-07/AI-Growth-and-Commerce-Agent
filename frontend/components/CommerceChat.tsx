  ...
  const payWithRazorpay = useCallback(async (msg: Message) => {
    ...
    const paymentSuccess = () => {
      setMessages(prev => [...prev, {
        id: `pays_${Date.now()}`,
        role: 'assistant',
        content: `✅ Payment verified successfully. Your order #${payment.order_id.slice(0, 8).toUpperCase()} has been confirmed (₹${payment.total.toLocaleString()}). Thank you for shopping with us!`,
        timestamp: new Date(),
      }])
      fetchCartCount()
    }
    ...
    try {
      if (payment.key_id) {
          ...
          handler: async (response: any) => {
            try {
              ...
              if (verifyRes.ok) {
                ...
                paymentFailed(`❌ Your payment was not completed. No successful charge was recorded. ${verifyData?.detail || 'Please try again.'}`)
              }
            } catch (e) {
        ...
        rzp.on('payment.failed', () => {
          paymentFailed('❌ Payment failed. Your order has NOT been marked as paid. You can retry checkout.')
          setPayingOrder(null)
        })
        rzp.open()
      } else {
        // Demo mode (no Razorpay keys configured) — labeled DEMO/SYNTHETIC.
        setTimeout(async () => {
          try {
            ...
            if (demoRes.ok) {
              ...
              paymentFailed('❌ Your payment was not completed. No successful charge was recorded. You can retry checkout.')
            }
          } catch (e) {
                    ...
                    {msg.payment && (
                      <div className="mt-3 p-3 bg-slate-50 rounded-lg border border-slate-200">
                        ...
                        <button
                          ...
                          className="w-full text-[11px] px-3 py-2.5 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-300 text-white rounded-lg transition-colors font-medium"
                        >
